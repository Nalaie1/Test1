...
public class UploadService : IUploadService
{
    ...
    public UploadService(IUploadRepository repo)
    {
        _repo = repo;
    }
    ...
    private static (string url, bool ok) SaveToDisk(byte[] bytes, string ext, IWebHostEnvironment env,
    ...
    private static async Task<(string url, bool ok)> SaveFileAsync(IFormFile file, IWebHostEnvironment env,
        string folderRelative)
    {
        var ext = Path.GetExtension(file.FileName).ToLowerInvariant();
        using var ms = new MemoryStream();
