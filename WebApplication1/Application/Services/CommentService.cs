...
public class CommentService : ICommentService
{
    ...
    {
        string cacheKey = $"CommentTree_{postId.ToString()}";
        if (!_cache.TryGetValue(cacheKey, out List<CommentDto> cachedTree))
        {
            var comments = await _repository.GetAllCommentsForPost(postId, includeReplies: true);
    ...
    {
        string cacheKey = $"CommentFlatten_{postId.ToString()}";
    ...
    private List<CommentDto> MapToCommentTree(List<Comment> comments)
    {
        ...
        {
            result.Add(MapCommentRecursive(comment, 0));
        }
    ...
    private List<CommentFlattenDto> FlattenComments(List<Comment> comments)
    {
        ...
        {
            FlattenRecursive(comment, commentDict, result, 0, "");
        }
